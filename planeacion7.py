# -*- coding: utf-8 -*-
"""
Created on Fri Mar  6 14:03:12 2026

@author: rjguz
"""

# gantt_15_weeks_professional_with_excel_format.py
...
st.set_page_config(
    page_title="Planning - Industrial Shield Predict AI", 
    page_icon="goat.jpg",
    layout="wide")
...
st.markdown("Developed by: Juan Rodrigo Guzmán Martínez")
...
st.sidebar.markdown(
    ...
    **Quick instructions:**  
    ...
    - Download the schedule as CSV or PNG.  
...
def to_excel(df):
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Activity Planning"

    # Title row
    ws.merge_cells('A1:G1')
    title_cell = ws['A1']
    title_cell.value = "Planeación de Actividades - Industrial Shield Predict"
    title_cell.font = Font(size=16, bold=True, color="FFFFFF")
    title_cell.fill = PatternFill("solid", fgColor="003366")  # Navy blue
    title_cell.alignment = Alignment(horizontal="center", vertical="center")

    # Header row
    headers = ["Phase", "Task", "Start Date", "End Date", "Description", "Deliverables"]
    ws.append(headers)
    for col_num, col_letter in enumerate(['A','B','C','D','E','F'], 1):
    ...
    # Data rows
    for r in dataframe_to_rows(df[["Phase", "Task", "Start", "Finish", "Description", "Deliverables"]], index=False, header=False):
        ws.append(r)

    # Format date columns
    for row in ws.iter_rows(min_row=3, min_col=3, max_col=4, max_row=ws.max_row):
        for cell in row:
    ...
    # Footer row with developer info
    ...
    footer_cell.value = "Developed by: Juan Rodrigo Guzmán Martínez"
    footer_cell.font = Font(italic=True, color="003366")
...
st.sidebar.download_button(
    ...
    data=excel_data,
    file_name='schedule_15_weeks_professional.xlsx',
